...
#* @apiTitle Pefindo Data prediction
#* @apiDescription Melakukan prediksi PD Score aplikasi loan nasabah
remove(list=ls())
options(warn=-1)
library(Rcpp)
library(plyr)
...
### Load Collateral
load("E://WORK/restrserve/DATA/collateral_columns.RData") 
### Load pefindo model
model_xgb_load = readRDS(file = "E://WORK/restrserve/DATA/model_xgb_24MAY2020_1436.RDS") 
### Load pefindo variable importance
xgb_importance = read.csv(file = "E://WORK/restrserve/DATA/xgb_importance_24MAY2020.csv") 
### Load SLIK model
model_xgb_SLIK = readRDS(file = "E://WORK/restrserve/DATA/Dev_Hit_model_xgb.RDS") 
### Load SLIK variable importance
xgb_importance_SLIK = read.csv('E://WORK/restrserve/DATA/xgb_importance_dimred.csv', stringsAsFactors = FALSE) 
### Load amenity data
amenity <- read.csv("E://WORK/restrserve/DATA/AMENITY.csv") 
### Load demographic model
model_xgb_demog = readRDS(file = "E://WORK/restrserve/DATA/model_cust_VERSION_D2_11JUNE.RDS") 
### Load banking model
model_banking = readRDS(file = "E://WORK/restrserve/DATA/model_bank_optimized_8JUNE1800.RDS")
### Load digital model
model_digital = readRDS(file = "E://WORK/restrserve/DATA/model_digital_optimized_25MAY2020_2248.RDS") 
### Calibration constants
calibrated_constants <- readRDS("E://WORK/restrserve/DATA/20200901_all_calibrated_constants.RDS")

#* Log waktu , jenis request serta user agent untuk request yang dilakukan
#* @filter logger
function(req){
...
function(res)
{
  if (res$status == 200)
  {
    ...
    return(list(error = "Not Available"))
  }
}


#*@param req POST request yang dilakukan oleh LOS, berisi data biro SLIK serta demografik nasabah
#*@post /slik
predict_slik <- function(req){
	Sys.sleep(10)
  ...
  input_to_json <- jsonlite::toJSON(input_data,auto_unbox = TRUE)
  input_to_json <- jsonlite::prettify(input_to_json)
  ...
  file_input <- as.character(paste("C://projects/",applicant_name,"-",file_date,"-",file_time,"-INPUT-SLIK.json",sep = ""))
  ...
  if(demog_data$CustomerType == "NTB" | is.na(demog_data$CIF) | demog_data$CIF ==""){
    ...
    con <- dbConnect(PostgreSQL(), dbname= "spark_beyond", host = "172.30.36.173", port = "5432",
  ...
  #simpan file output
  file_output <- as.character(paste("C://projects/",applicant_name,"-",file_date,"-",file_time,"-OUTPUT-SLIK.json",sep = ""))
  jsonlite::write_json(json_output_save, file_output)
  return(json_output)
}


#* @param req POST request yang dilakukan oleh LOS, berisi data pefindo, demografik dan data aplikasi nasabah
#* @post /pefindo
predict_pefindo <- function(req){
  input_data <- jsonlite::fromJSON(req$postBody)
  app_data <- input_data [["DemographicData"]] # ambil data application dari json input
  ...
  file_date <- as.character(Sys.Date())
  file_time <- as.character(format(Sys.time(),"%H%M%S"))
  file_input <- as.character(paste("C://projects/",applicant_name,"-",file_date,"-",file_time,"-INPUT.json",sep = ""))
  write(input_to_json,file = file_input)
  ...
  #simpan file output
  file_output <- as.character(paste("C://projects/",applicant_name,"-",file_date,"-",file_time,"-OUTPUT.json",sep = ""))
  jsonlite::write_json(json_output_save, file_output)
  return(json_output)
}
...
########################################################################################################
#------------------------------ PEF1.FUNGSI UNTUK BUAT TABEL PEFINDO  ---------------------------------#
########################################################################################################
...
########################################################################################################
#-------------------------- PEF2.FUNGSI PERHITUNGAN PD SCORE BIRO MODULE PEFINDO -----------------------------#
########################################################################################################
...
########################################################################################################
#----------------------- SLK1.FUNGSI PERHITUNGAN PD SCORE BIRO MODULE SLIK --------------------------#
########################################################################################################
...
#####################################################################################################
#-------------------------- DEM1.FUNGSI UNTUK MENGHITUNG MODUL DEMOGRAFIK -------------------------------#
######################################################################################################
...
#####################################################################################################
#-------------------------- BNK1.FUNGSI UNTUK MENGHITUNG MODUL BANKING -------------------------------#
######################################################################################################
...
#####################################################################################################
#-------------------------- DIG1.FUNGSI UNTUK MENGHITUNG MODUL DIGITAL -------------------------------#
######################################################################################################
...
#####################################################################################################
#--------------------------------- NTB1.FUNGSI PD SCORE GABUNGAN UNTUK NTB -----------------------------------#
######################################################################################################
...
#####################################################################################################
#--------------------------------- ETB1. FUNGSI PD SCORE GABUNGAN UNTUK ETB -----------------------------------#
######################################################################################################


# ------------------------------------SCORING ETB UNTUK PEFINDO--------------------------------------#
